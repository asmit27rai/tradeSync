...
interface IWarden {
  ...
  /// @param rejectReason The reject reason
  /// @return success If execution was successful
  function rejectSignRequest(uint64 requestId, string calldata rejectReason) external returns (bool success);
  ...
  function keyRequestById(uint64 id) external view returns (KeyRequest memory keyRequest);
